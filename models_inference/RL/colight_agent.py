# ============================================================================
# PORT GLUE (NOT in source) -- keep this block separate so the body below stays
# byte-diffable against the pinned source:
#   github.com/usail-hkust/LLMTSCS @ d5d4180f34edb843e1d1b462d5846c75d6d4533a
#   models/colight_agent.py
#
# The source targets tensorflow-cpu==2.8.0, whose `tensorflow.keras` IS Keras 2.
# Modern TF ships Keras 3 by default, which rejects the verbatim Keras-2 idioms
# below (Adam(lr=...), K.* backend ops, model_from_json custom_objects, Lambda
# serialization). Installing the `tf-keras` package and setting this env var make
# `tensorflow.keras` resolve back to Keras 2, so the lifted code runs unchanged.
# Must be set BEFORE tensorflow is first imported.
import os as _os
_os.environ.setdefault("TF_USE_LEGACY_KERAS", "1")
#
# Newer tf-keras defaults to safe_mode=True, which refuses to deserialize the
# Lambda layers this network uses (TF 2.8 had no such guard). build_network_from_copy
# round-trips through model_from_json, so we flip the documented global toggle once,
# leaving the verbatim load/copy code untouched.
try:
    from tf_keras.src.saving.serialization_lib import enable_unsafe_deserialization as _enable_unsafe
    _enable_unsafe()
except Exception:  # toggle path differs across tf-keras versions; load calls will surface it
    pass
#
# The ONLY in-body deviation from source is one line in _cal_len_feature (the
# lane-feature width), made config-driven so 8-movement SUMO state fits; it is
# marked inline with `# PORT:` and defaults to source's 12 when the key is absent.
# ============================================================================
"""
Colgiht agent.
    ...
    def _cal_len_feature(self):
        ...
        for feat_name in used_feature:
            if "cur_phase" in feat_name:
                ...
                # PORT: source hardcodes `N += 12` (CityFlow 3-lane = 12 entering lanes).
                # SUMO state uses an 8-movement vector; width is read from config and
                # defaults to source's 12 when the key is absent (so the diff is inert
                # for source-shaped configs).
                N += self.dic_traffic_env_conf.get("NUM_LANE_FEATURES", 12)
        return N
